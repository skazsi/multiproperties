...
public class ColumnsMasterDetailsBlock extends MasterDetailsBlock {
	...
	public void refreshButtons() {
		...
		if (selection.isEmpty()) {
			...
			down_button.setEnabled(
					editor.getTable().getColumns().indexOf(column) < editor.getTable().getColumns().size() - 1);
		}
	}
	...
	protected void createMasterPart(final IManagedForm managedForm, final Composite parent) {
		...
		section.setText(Messages.getString("general.columns"));
		...
		// Add
		add_button = toolkit.createButton(buttonComposite, Messages.getString("columns.button.add"), SWT.PUSH);
		...
		// Remove
		remove_button = toolkit.createButton(buttonComposite, Messages.getString("columns.button.remove"), SWT.PUSH);
		...
		remove_button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent e) {
				...
				if (selection.isEmpty())
					return;
				final Column column = (Column) ((IStructuredSelection) selection).getFirstElement();
				if (MessageDialog.openConfirm(editor.getSite().getShell(), Messages.getString("general.confirm.title"),
						Messages.getString("columns.button.remove.confirm.text"))) {
		...
		// Up
		up_button = toolkit.createButton(buttonComposite, Messages.getString("columns.button.up"), SWT.PUSH);
		...
		// Down
		down_button = toolkit.createButton(buttonComposite, Messages.getString("columns.button.down"), SWT.PUSH);
		...
		toolkit.createLabel(buttonComposite, "");
		// Import
		import_button = toolkit.createButton(buttonComposite, Messages.getString("columns.button.import"), SWT.PUSH);
	...
	protected void createToolBarActions(final IManagedForm managedForm) {
		...
		final Action horizontalAction = new Action(Messages.getString("columns.button.horizontal"),
		...
		horizontalAction.setToolTipText(Messages.getString("columns.button.horizontal"));
		horizontalAction.setImageDescriptor(Activator.getImageDescriptor("icons/th_horizontal.gif"));
		final Action verticalAction = new Action(Messages.getString("columns.button.vertical"),
				Action.AS_RADIO_BUTTON) {
		...
		verticalAction.setToolTipText(Messages.getString("columns.button.vertical"));
		verticalAction.setImageDescriptor(Activator.getImageDescriptor("icons/th_vertical.gif"));
