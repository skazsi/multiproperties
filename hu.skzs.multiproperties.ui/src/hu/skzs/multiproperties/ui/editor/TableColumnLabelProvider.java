...
/**
 * The <code>TableColumnLabelProvider</code> is the label provider for the table
 * page.
 *
 * @author skzs
 */
public class TableColumnLabelProvider extends ColumnLabelProvider {
	...
	/**
	 * The <code>column</code> member hold a reference to the {@link Column}
	 * instance.
	 * <p>
	 * When it is <code>null</code>, the label provider defines how the
	 * <strong>key</strong> column should be displayed.
	 * </p>
	 */
	 ...
	 * <p>
	 * The label provider constructed in this way defines how the
	 * <strong>key</strong> column should be displayed.
	 * </p>
	 */
	 ...
	 * <p>
	 * The label provider constructed in this way defines how the given
	 * {@link Column} should be displayed.
	 * </p>
	 *
	...
	private String getImageName(final PropertyRecord propertyRecord) {
		final StringBuilder imageName = new StringBuilder("record");
		if (propertyRecord.isMultiLine()) {
			imageName.append("_multiline");
		}
		if (propertyRecord.isDisabled()) {
			imageName.append("_disabled");
		}
		if (propertyRecord.isDuplicated()) {
			imageName.append("_duplicated");
		}
	...
	private Image createDecoratedImage(final PropertyRecord propertyRecord) {
		final ImageRegistry registry = Activator.getDefault().getImageRegistry();
		Image image = registry.get("record");
		if (propertyRecord.isMultiLine()) {
			image = createDecoratedImage(image, registry.get("multiline"), IDecoration.BOTTOM_RIGHT);
		}
		if (propertyRecord.isDisabled()) {
			image = createDecoratedImage(image, registry.get("disabled"), IDecoration.BOTTOM_LEFT);
		}
		if (propertyRecord.isDuplicated()) {
			image = createDecoratedImage(image, registry.get("warning"), IDecoration.TOP_RIGHT);
		}
		return image;
	}

	/**
	 * Return a new {@link Image} instance by decorating the given <code>base</code>
	 * image by the given <code>decorator</code> image in the given
	 * <code>quadrant</code>.
	 *
	 * @param base
	 * @param decorator
	...
	public Color getForeground(final Object element) {
		...
		if (element instanceof PropertyRecord) {
			...
			if (propertyRecord.getDefaultColumnValue() != null)
				if (propertyRecord.isDisabled())
					return parseColor(
							store.getString(PreferenceConstants.COLOR_DISABLED_PROPERTY_DEFAULTVALUE_FOREGROUND));
				else
	...
	/**
	 * Returns a newly constructed {@link Color} instance based on the given
	 * <code>value</code> holding a <code>R,G,B</code> value.
	 *
	...
	private Color parseColor(final String value) {
		// TODO: The created Color instances should be disposed after the usage. Some
		// color registry should be used maybe.
	 ...
	 * Otherwise, when its trimmed value equals to an empty String <code>null</code>
	 * is returned.
	 *
